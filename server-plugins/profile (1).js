...
exports.commands = {
  ...
  team: {
    ...
    set: function (target, room, user) {
		...
		if (!target) return this.parse('/team help');
    ...
    take: function (target, room, user) {
		if (!this.can('broadcast')) return false;
		if (!target) return this.errorReply('USAGE: /team take [USER]'); /*
		let user = target.toLowerCase().trim(); */
    ...
    help: function (target, room, user) {
     ...
     'All commands are nestled under namespace <code>team</code></center><br>' +
     '<hr width="100%">' +
			...
			'For example: Mega Venusaur would be 003-Mega<br>' +
	...
	crush: {
		...
		delete: function (target, room, user) {
			...
			if (!target) {
				...
				let userid = toId(target);
				if (!Db("friendcode").has(userid)) return this.errorReply(userid + " hasn't set a crush.");
		...
		function showProfile() {
			Economy.readMoney(toId(username), money => {
				...
				if (Db("friendcode").has(toId(username))) {
					profile += '&nbsp;<font color="#24678d"><strong>Crush:</strong></font> ' + Db("friendcode").get(toId(username)) + '<br />';
				}
